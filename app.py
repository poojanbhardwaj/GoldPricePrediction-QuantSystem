...
if page == "Market Research Assistant":
    ...
    if run_full_clicked or refresh_market_clicked:
        with st.status("Building the final research snapshot...", expanded=True) as run_status:
            ...
            try:
                ...
                # Refresh/rebuild can rewrite data/processed/master_dataset.csv.
                # Clear both cached layers so dashboard cards read the newly saved market data.
                try:
                    _load_cached_market_history.clear()
                except Exception:
                    pass
                try:
    ...
    if not _has_real_phase29_predictions(phase29_snapshot):
        st.warning(
            "Prediction snapshot unavailable. Showing latest available market prices only. Refresh / rebuild the "
            "research snapshot or check saved forecast artifacts."
        )
        # Keep a freshly refreshed/session price snapshot visible instead of falling back
        # to an older cached CSV after a successful data refresh.
        if _phase29_displayable_snapshot(phase29_snapshot):
            st.session_state.phase29_snapshot_source = st.session_state.get("phase29_snapshot_source", "session")
            phase29_snapshot = _phase29_placeholder_snapshot(phase29_snapshot)
        else:
            st.session_state.phase29_snapshot_source = "placeholder"
            try:
                _load_cached_market_history.clear()
